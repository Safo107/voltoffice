...
export async function POST(req: NextRequest) {
  try {
    ...
    if (doc.preis && doc.projektId && doc.preis > 0) {
      try {
        await upsertMaterialInvoice(db, doc);
      } catch {
...
async function upsertMaterialInvoice(
  ...
  // Projekt laden für Kundenname
  const { ObjectId } = await import("mongodb");
  let customerName = "—";
  let projektTitel = "";
  try {
    const proj = await db.collection("projekte").findOne({ _id: new ObjectId(material.projektId) });
  ...
  // Bestehende Entwurfs-Rechnung für dieses Projekt suchen
  ...
  if (existing) {
    // Existierende Rechnung aktualisieren
    ...
    // Neue Entwurfs-Rechnung erstellen
